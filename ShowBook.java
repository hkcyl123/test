...
//    static MainWin mw;
    ...
    public void showFirst(String s){
        ...
        for(int i = 0; i < count; i++){
            System.arraycopy(array[0][i], 0, MainWin.ar[i], 0, 10);
        }
        ...
        if(maxPage> 0){
        	if(count == 1){
            MainWin.nextButton = false;
            MainWin.lastButton = false;
            }else if(count > 1){
    ...
    public void showPage(int n){
        ...
        //MainWin.ar = array[n];
        for(int i = 0; i < ROW; i++){
            System.arraycopy(array[n][i], 0, MainWin.ar[i], 0, 10);
        }
        if(count == n){
        	MainWin.nextButton = false;
        	MainWin.lastButton = false;
        	MainWin.firstButton = true;
        	MainWin.prevButton = true;
        }else{
        	MainWin.nextButton = true;
        	MainWin.lastButton = true;
        	MainWin.firstButton = false;
        	MainWin.prevButton = false;
        }
        MainWin.table.repaint();
        //new MainWin(MainWin.userName).setVisible(true);
    ...
    private String[][][] get(String s){
        ...
        int page=0;
        count=0;
        String ar[][][] = new String[PAGE][ROW][10];
            ...
            for(int i = 0; i < PAGE; i++){
                for(int j = 0; j < ROW; j++){
                    if(rs.next()){
                        ...
                        ar[page][j][7] = rs.getString(8);
                        ar[page][j][8] = rs.getString(9);
                        ar[page][j][9] = rs.getString(10);
                        count++;
                    }
                    else{
                        j = ROW;
