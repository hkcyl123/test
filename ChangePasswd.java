...
class ChangePasswd
    ...
    public void updatePasswd(){
        ...
        db.dbCon();
        try{
            String query = "select * from administrator where MID='" + username + "'";
            ResultSet rs = db.stmt.executeQuery(query);
            if(rs.next()){
                String oldPasswd = rs.getString(3);
                String newPasswd = new_passwd.getText().trim();
                String a = old_passwd.getText().trim();
                if(a.equals(oldPasswd)){
                    String s = "update administrator set Mpsw='" + newPasswd
                            + "' where MID='" + username + "'";
