...
public class DelBook{
    ...
    public void delBook(){
        ...
        if(index.length == 0) //if (index.length == 0 || index.length == 0 && MainWin.table.getValueAt(int[0], 0) == "")
        {
            ...
            int selection = JOptionPane.showConfirmDialog(null, "确定要删除所选中图书吗？",
                ...
                try{
                    ...
                    for(int i = 0; i < index.length; ++i){
                        String str_v = (String) MainWin.table.getValueAt(
                                index[i], 1);
                        String s = "delete from bookno where bookNo ='" + str_v
                                + "'";
                    ...
                    if(n == index.length){
                        ...
                        JOptionPane.showMessageDialog(null, n + " 条记录被删除，"
                                + (index.length - n) + " 条记录无法删除！",
                                "信息", JOptionPane.YES_NO_OPTION);
                    }
